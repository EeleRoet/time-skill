...
class CustomTimeSkill(MycroftSkill):

    def __init__(self):
        super(CustomTimeSkill, self).__init__("CustomTimeSkill")
        self.displayed_time = None
        self.display_tz = None
        self.answering_query = False
        self.default_timezone = "Europe/Amsterdam"
    ...
    # custom intent: query our timezone
    @intent_handler('timezone.our.intent')
    def handle_query_our_timezone(self, message):
        self.writeLogs(message, inspect.stack()[0][3])
        ourTimezone = self._get_timezone_from_builtins(self.default_timezone)
        utcOffset = self.utc_offset_by_timezone(ourTimezone)
        aheadOrBehind = 'ahead of' if utcOffset >= 0 else 'behind'
        self.speak_dialog('our.timezone', {'timezone': ourTimezone.zone, 'hours': utcOffset, 'aheadOrBehind': aheadOrBehind})

    # custom intent: query other timezone
    @intent_handler('timezone.elsewhere.intent')
    def handle_query_other_timezone(self, message):
        self.writeLogs(message, inspect.stack()[0][3])
        location = message.data.get('location')
        timezone = self._get_timezone_from_builtins(location)

        if( timezone == None):
            location = self.get_response('prompt.location', validator=self.location_prompt_validator, num_retries=2)

            self.log.info("location from prompt = {0}".format(location))

            if(location != None):
                timezone = self._get_timezone_from_builtins(location)
            else:
                self.speak_dialog('timezone.unknown')
                return

        ourTimezone = self._get_timezone_from_builtins(self.default_timezone)
        ourUtcOffset = self.utc_offset_by_timezone(ourTimezone)
        otherUtcOffset = self.utc_offset_by_timezone(timezone)
        utcOffsetDifference = otherUtcOffset - ourUtcOffset

        aheadOrBehind = 'ahead of' if utcOffsetDifference >= 0 else 'behind'

        self.speak_dialog('other.timezone', {'location': location, 'timezone': timezone.zone, 'hours': utcOffsetDifference, 'aheadOrBehind': aheadOrBehind})

    def utc_offset_by_timezone(self, timezone):
        return timezone.localize(datetime.datetime.now()).utcoffset().seconds / 3600 + 24 * timezone.localize(datetime.datetime.now()).utcoffset().days

    def location_prompt_validator(self, response):
        timezone = self.get_timezone(response)
